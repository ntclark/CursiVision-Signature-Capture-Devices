...
{                                                             \
   ...
   wchar_t wszFiles[][64] = {L"pkAPI.ocx",L"WacomCV.dll",L"signotecCV.dll",L"ScriptelCV.dll",L"TabletPC.ocx"};
   ...
   case WM_INITDIALOG: {
      ...
      for ( long k = IDDI_PAD_MODEL_MIN; k <= IDDI_PAD_MODEL_MAX; k++ ) {
         ...
         if ( strstr(szModel,p -> signatureDeviceProductName) || 0 == strncmp(szModel,"Wacom",5) ) {
   ...
   case WM_COMMAND: {
      ...
      case IDDI_PAD_ORIGINPOINT_SET: {
         DialogBoxParam(hModule,MAKEINTRESOURCE(IDD_PAD_ORIGINPOINT),hwnd,(DLGPROC)SignaturePad::originPointHandler,(long)p);
         }
         break;

      case IDDI_INK_PROPERTIES_SET: {
         DialogBoxParam(hModule,MAKEINTRESOURCE(IDD_PAD_INK_PROPERTIES),hwnd,(DLGPROC)SignaturePad::inkHandler,(long)p);
         }
         break;
