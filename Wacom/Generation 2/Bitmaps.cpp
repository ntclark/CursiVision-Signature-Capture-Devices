...
#include "SignaturePad.h"
   ...
   if ( ! ( NULL == sendingBitmapThreadHandle ) )
      WaitForSingleObject(sendingBitmapThreadHandle,INFINITE);
   ...
   case STU_300:
   case STU_500: {
      std::vector<uint8_t> fwacom = WacomGSS::STU::ProtocolHelper::flattenMonochrome(bm,pTabletCapability.screenWidth,pTabletCapability.screenHeight);
      pTablet -> writeImage(WacomGSS::STU::Protocol::EncodingMode_1bit,(uint8_t *)fwacom.data(),fwacom.size());
      }
      break;

   case STU_520:
      //pTablet -> writeImage(WacomGSS::STU::Protocol::EncodingMode_16bit,(uint8_t *)pBits,entireSize);
      break;
   }
