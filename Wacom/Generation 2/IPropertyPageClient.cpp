...
#include "SignaturePad.h"
   ...
   *pCntPages = 1;
   ...
   if ( 0 == pageNumber ) {
      ...
      pParent -> hwndSettings = CreateDialogIndirectParam(hModule,dt,(HWND)hwndParent,(DLGPROC)SignaturePad::settingsHandler,(LPARAM)pParent);
   ...
   *pResult = S_FALSE;
   ...
   PROPSHEETHEADERA *pHeader = reinterpret_cast<PROPSHEETHEADERA *>(pv);
   ...
   *pCount = 1;
   ...
   PROPSHEETPAGEA *pPropSheetPages = reinterpret_cast<PROPSHEETPAGEA *>(pPages);

   pPropSheetPages[0].dwFlags = PSP_USETITLE;
   pPropSheetPages[0].dwSize = sizeof(PROPSHEETPAGE);
   pPropSheetPages[0].hInstance = hModule;
   pPropSheetPages[0].pszTemplate = MAKEINTRESOURCEA(IDD_PAD_PROPERTIES);
