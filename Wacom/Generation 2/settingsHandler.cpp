...
#define PUT_LONG(v,id)  { char szX[32]; sprintf(szX,"%ld",v); SetDlgItemTextA(hwnd,id,szX); }
#define PUT_DOUBLE(v,id) { char szX[64]; sprintf(szX,"%5.2lf",v); SetDlgItemTextA(hwnd,id,szX); }
...
#define GET_LONG(v,id) {char szX[32]; GetDlgItemTextA(hwnd,id,szX,32); v = atol(szX); }
#define GET_DOUBLE(v,id) {char szX[64]; GetDlgItemTextA(hwnd,id,szX,64); v = atof(szX); }
...
{                                                             \
   ...
   case WM_INITDIALOG: {
      ...
      for ( long k = IDDI_PAD_MODEL_MIN; k <= IDDI_PAD_MODEL_MAX; k++ ) {
         char szModel[128];
         GetWindowTextA(GetDlgItem(hwnd,k),szModel,128);
      ...
      LoadStringA(hModule,IDDI_PAD_MODEL_CHOICE_LABEL2,szTemp,sizeof(szTemp));
      SetDlgItemTextA(hwnd,IDDI_PAD_MODEL_CHOICE_LABEL2,szTemp);
      ...
      if ( ! isAdministrator ) {
         LoadStringA(hModule,IDDI_PAD_MODEL_CHOICE_NONADMIN,szTemp,sizeof(szTemp));
         ...
         SetDlgItemTextA(hwnd,IDDI_PAD_MODEL_CHOICE_NONADMIN,szTemp);
   ...
   case WM_INITDIALOG: {
      ...
      gClass.lpszClassName = L"PadImage";

      RegisterClass(&gClass);

      hwndPadImage = CreateWindowEx(0*WS_EX_CLIENTEDGE,L"PadImage",L"",WS_CHILD | WS_VISIBLE, 8, 64, cx, cy, hwnd, NULL, hModule,(void *)lParam);
...
#define REFRESH_ORIGIN_POINT                                                                                                     \
   ...
   case WM_INITDIALOG: {
      ...
      SetWindowTextA(GetDlgItem(hwnd,IDDI_INK_PROPERTIES_WEIGHT),szX);
      ...
      HWND weightSpinner = CreateWindowEx(0L,UPDOWN_CLASS,L"",WS_VISIBLE | WS_CHILD | UDS_SETBUDDYINT,rcWeight.right + dx + 2,rcWeight.top + dy + 2,30,cyWeight,
   ...
   case WM_COMMAND:
      ...
      case IDDI_INK_PROPERTIES_OK:
         char szX[32];
         GetWindowTextA(GetDlgItem(hwnd,IDDI_INK_PROPERTIES_WEIGHT),szX,32);
         p -> penWidth = atol(szX);
         EndDialog(hwnd,1L);
         break;
   ...
   case WM_PAINT: {
      ...
      BeginPaint(hwnd,&ps);
      char szText[1024];
      GetWindowTextA(hwnd,szText,1024);
      HFONT hGUIFont = (HFONT)GetStockObject(DEFAULT_GUI_FONT);
      SelectObject(ps.hdc,hGUIFont);
      SetTextColor(ps.hdc,RGB(255,0,0));
      SetBkColor(ps.hdc,GetSysColor(COLOR_MENU));
      DrawTextA(ps.hdc,szText,(DWORD)strlen(szText),&ps.rcPaint,DT_TOP);
      EndPaint(hwnd,&ps);
      }
      break;
