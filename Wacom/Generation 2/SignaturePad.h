...
class SignaturePad : public ISignaturePad {
   ...
   private:
      ...
      char * __stdcall DeviceProductName() { return STR_WACOM_MODEL; };
...
//      IConnectionPointContainer
   ...
   private:
      ...
      //
      // Items related to the Wacom signature pads and it's interface
      //

      static Tablet *pTablet;
      static std::unique_ptr<UsbInterface> pUsbInterface;
      static Protocol::Capability pTabletCapability;
      static Protocol::Information pTabletInformation;
      ...
      class CVReportHandler : public WacomGSS::STU::ProtocolHelper::ReportHandler {

      public:
         CVReportHandler(SignaturePad *pParent) : pSignaturePad(pParent) {};

      private:

         void onReport(Protocol::PenData &penData);

         SignaturePad *pSignaturePad;

      };

      static unsigned int __stdcall SignaturePad::fireOptionThreaded(void *pv);

      //
      // End of items for the Wacom pad
      //
