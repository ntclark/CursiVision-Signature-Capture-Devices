...
#if 0
   HDC hdcMonochrome = CreateCompatibleDC(hdc);
   HBITMAP hBitmapMonochrome = CreateBitmap(cx,cy,1,1,NULL);
   HGDIOBJ oldBitmap = SelectObject(hdcMonochrome,hBitmapMonochrome);
   HGDIOBJ oldBitmap2 = SelectObject(hdc,hBitmap);

   long rc = BitBlt(hdcMonochrome,0,0,cx,cy,hdc,0,0,SRCCOPY);

   SelectObject(hdcMonochrome,oldBitmap);
   SelectObject(hdc,oldBitmap2);
#endif

   STDisplaySetImage(x,y,hBitmapMonochrome);
#else
   STDisplaySetImage(x,y,hBitmap);
#endif
   return;
   }
